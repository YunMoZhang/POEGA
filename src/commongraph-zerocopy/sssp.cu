...
int main(int argc, char** argv)
{
	...
	while (graph.numActiveNodes>0)
	{
		...
		timer_sub.Start();	
		sssp_kernel_common<<<graph.num_nodes / 512 + 1, 512>>>(graph.num_nodes,
																  graph.d_nodePointer,
																  graph.d_edgeList,
																  graph.d_value,
																  graph.d_label1,
	...
	for (uint i = 0; i < graph.numSnapshots; i++){
		sssp_incremental_cg<<<graph.num_nodes / 512 + 1, 512, 0, streams[i]>>>(graph.num_nodes,
																	graph.d_nodePointer,
																	graph.d_edgeList,
																	d_label2s[i],
																	d_values[i],
	...
	while (graph.numActiveNodes > 0)
	{
		...
		for (uint i = 0; i < graph.numSnapshots; i++){
			...
			sssp_kernel<<<graph.num_nodes / 512 + 1, 512, 0, streams[i]>>>(graph.num_nodes,
																graph.d_nodePointer,
																graph.d_edgeList,
																d_label1s[i],
																d_label2s[i],
