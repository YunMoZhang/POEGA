...
int main(int argc, char** argv)
{
	...
	while (graph.numActiveNodes>0)
	{
		...
		timer_sub.Start();	
		bfs_kernel_ks<<< graph.num_nodes/512 + 1 , 512 >>>(graph.num_nodes,
													graph.d_nodePointer,
													graph.d_edgeList,
													graph.d_outDegree,
													graph.d_label1,
	...
	for(uint i=1; i<=graph.numSnapshots;i++)
	{
		...
		while(graph.numActiveNodes>0){
			...
			gpuErrorcheck(cudaPeekAtLastError());	
			// timer_sub3.Start();
			bfs_jumpstart<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_nodes, 
															graph.d_nodePointer, 
															graph.d_edgeList, 
															graph.d_outDegree, 
															graph.d_label1, 
			...
			gpuErrorcheck(cudaPeekAtLastError());	
			// timer_sub3.Start();
			bfs_propogate<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_nodes, 
															graph.d_nodePointer, 
															graph.d_edgeList, 
															graph.d_outDegree, 
															graph.d_label1, 
		...
		timer_sub2.Start();
		bfs_pull_once<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_nodes, 
														graph.d_nodePointer, 
														graph.d_edgeList, 
														graph.d_outDegree, 
														graph.d_depends,
