...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			sssp_kernel_cg<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	subgen.generate(graph, subgraph);
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			for(uint sid = 0; sid < graph.numSnapshots; sid++){
				sssp_incremental_kernel<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, streams[sid]>>>(partitioner.partitionNodeSize[pi],
																								  partitioner.fromNode[pi],
																								  partitioner.fromEdge[pi],
																								  subgraph.d_activeNodes,
																								  subgraph.d_activeNodesPointer,
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			for(uint sid = 0; sid < graph.numSnapshots; sid++){
				sssp_kernel<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, streams[sid]>>>(partitioner.partitionNodeSize[pi],
																								  partitioner.fromNode[pi],
																								  partitioner.fromEdge[pi],
																								  subgraph.d_activeNodes,
																								  subgraph.d_activeNodesPointer,
