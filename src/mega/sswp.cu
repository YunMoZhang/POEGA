...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			sswp_kernel_cg<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			sswp_kernel_concurrent_cg<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
																							partitioner.fromNode[pi],
																							partitioner.fromEdge[pi],
																							subgraph.d_activeNodes,
																							subgraph.d_activeNodesPointer,
