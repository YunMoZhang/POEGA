...
int main(int argc, char** argv)
{
	...
	/// Init computation
	float total_comm = 0, total_comp = 0, total_gen = 0;
	timer.Start();
	uint itr = 0;
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			bfs_kernel_cg<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	for (uint i = 0; i < graph.numSnapshots; i++)
	{
		...
		// gpuErrorcheck(cudaMemcpy(d_label2s[i], graph.core_active_vertices, graph.num_nodes * sizeof(bool), cudaMemcpyHostToDevice));
		sssp_setActive<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_value, d_label2s[i], graph.num_nodes);
	}
	...
	subgen.generate(graph, subgraph);
	{
		...
		timer_sub.Start();
		cout << "Incre with #nodes " << subgraph.numActiveNodes << " out of " << graph.num_nodes << "\t";
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			for(uint sid = 0; sid < graph.numSnapshots; sid++){
				bfs_incremental_kernel<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, streams[sid]>>>(partitioner.partitionNodeSize[pi],
																								  partitioner.fromNode[pi],
																								  partitioner.fromEdge[pi],
																								  subgraph.d_activeNodes,
																								  subgraph.d_activeNodesPointer,
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			// sssp_kernel_concurrent_cg<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
			// 																					  partitioner.fromNode[pi],
			// 																					  partitioner.fromEdge[pi],
			// 																					  subgraph.d_activeNodes,
			// 																					  subgraph.d_activeNodesPointer,
			...
			// 																					  graph.numSnapshots);
			for(uint sid = 0; sid < graph.numSnapshots; sid++){
				bfs_kernel<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, streams[sid]>>>(partitioner.partitionNodeSize[pi],
																								  partitioner.fromNode[pi],
																								  partitioner.fromEdge[pi],
																								  subgraph.d_activeNodes,
																								  subgraph.d_activeNodesPointer,
