...
{	
	...
	// this->max_partition_size = 0.3 * (dev.totalGlobalMem - 28 * 4 * (size_t)num_nodes) / sizeof(E);
	this->max_partition_size = 0.3 * (dev.totalGlobalMem - 35 * 4 * (size_t)num_nodes) / sizeof(E);
