...
int main(int argc, char** argv)
{
	...
	for (uint i = 0; i < graph.num_nodes; i++)
	{
		...
		if (graph.outDegree[i] > 0){
			graph.label2[i] = true;
			graph.numActiveNodes++;
		}
		else
			graph.label2[i] = false;
		graph.value[i] = i;
	}
	cout << graph.numActiveNodes << " active nodes in the beginning.\n";
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			cc_kernel_cg<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	for (uint i = 0; i < graph.numSnapshots; i++)
	{
		...
		cc_setActive<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_value, d_label2s[i], graph.num_nodes);
	}

	timer.Start();
	cc_setActive<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_value, graph.d_label2, graph.num_nodes);
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			for(uint sid = 0; sid < graph.numSnapshots; sid++){
				cc_kernel_cg<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, streams[sid]>>>(partitioner.partitionNodeSize[pi],
																								  partitioner.fromNode[pi],
																								  partitioner.fromEdge[pi],
																								  subgraph.d_activeNodes,
																								  subgraph.d_activeNodesPointer,
