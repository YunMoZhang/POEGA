...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			viterbi_kernel_cg<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
	...
	timer.Start();
	sswp_setActive<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_value, graph.d_label2, graph.num_nodes);
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			viterbi_kernel_concurrent_cg<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
																							partitioner.fromNode[pi],
																							partitioner.fromEdge[pi],
																							subgraph.d_activeNodes,
																							subgraph.d_activeNodesPointer,
