...
void two_stage_analysis(Evolving_Graph<OutEdgeWeighted_Evolving, EdgeWeighted_Union, EdgeWeighted> &graph, ArgumentParser& arguments, int seed){
	...
	float sub_runtime, analysis_time, sub_time;
	...
	cout << "Analyzing the first csg" << endl;
	for(uint i = 0; i < graph.num_nodes; i++){
		graph.label1[i] = false;
		graph.label2[i] = true;
		graph.value[i] = i;
	}
	graph.numActiveNodes = graph.num_nodes;
	...
	while (graph.numActiveNodes > 0)
	{
		...
		cc_kernel_common<<<(graph.num_nodes) / 512 + 1, 512>>>(graph.num_nodes,
	...
	cout << ">>Processing Common graph finished in " << runtime << " (ms).\n";
	...
	#pragma omp parallel for
		...
		gpuErrorcheck(cudaPeekAtLastError());
		sub_timer.Start();
		cc_incremental_cg_concurrent2<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								d_values,
																								graph.d_label2,
		...
		while (graph.numActiveNodes > 0)
		{
			...
			timer_sub2.Start();
			moveUpLabels<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			cc_kernel_ks_concurrent2<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_label1,
																								graph.d_label2,
		...
		gpuErrorcheck(cudaMemcpy(value_data, d_values, graph.num_nodes * sizeof(DependencyData), cudaMemcpyDeviceToHost));
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			bfs_incre_concurrent_locality<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											d_values,
																											graph.d_label2,
																											graph.numSnapshots);
			cc_incre_concurrent2_locality<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											d_values,
																											graph.d_label2,
		...
		cout << "P2-1 tracing time: " << total_time << endl;
	...
	{	
		uint *d_buffer;
		cudaMalloc(&d_buffer, (graph.num_nodes * 0.4) * graph.numSnapshots  * sizeof(uint));
		...
		cudaMemset(d_total, (graph.num_nodes * 0.4), sizeof(uint));
		cout << "-- buffer init size: "<<(graph.num_nodes * 0.4)  << endl;

		timer.Start();
		clearLabel<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.num_nodes);
		gpuErrorcheck(cudaPeekAtLastError());
		sub_timer.Start();
		cc_incremental_cg_concurrent2_newstorage<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_value,
																								d_buffer,
		...
		while (graph.numActiveNodes > 0)
		{
			...
			timer_sub2.Start();
			moveUpLabels<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			cc_kernel_concurrent_newstorage<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_label1,
																								graph.d_label2,
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			extend_varray_to_edgelist<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionNodeSize[pi],
																									...
																									d_nodePointer_el,
																									graph.d_outDegree);
			cc_incre_concurrent_locality_newstorage<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																										d_nodePointer_el,
																										d_edgeList,
																										graph.d_value,
																										d_buffer,
																										...
																										d_total,
																										graph.numSnapshots);
			cc_incre_concurrent2_locality_newstorage<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											graph.d_value,
																											d_buffer,
		...
		gpuErrorcheck(cudaMemcpy(&h_counter, d_counter, sizeof(uint), cudaMemcpyDeviceToHost));
		cout << "--buffer counter: " << h_counter << endl;
		if(h_counter >= graph.num_nodes * 0.4)
			exit(0);
	...
	for(uint top_i = 0; top_i < graph.top_k_degree; top_i++){
		...
		while (subgraph.numActiveNodes > 0)
		{
			...
			for (int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				cc_kernel<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
																				partitioner.fromNode[pi],
																				partitioner.fromEdge[pi],
