...
__global__ void prePrefix(unsigned int *activeNodesLabeling, unsigned int *activeNodesDegree,
						  unsigned int *outDegree, bool *label1, bool *label2, unsigned int numNodes, uint maxDegreeThreshold)
{
	unsigned int id = blockDim.x * blockIdx.x + threadIdx.x;
	if (id < numNodes)
	{
		activeNodesLabeling[id] = label1[id] || label2[id]; // label1 is always zero in sync
		if(activeNodesLabeling[id] && outDegree[id] >= 2 * maxDegreeThreshold){
			activeNodesLabeling[id] = outDegree[id] / maxDegreeThreshold;
		}
		activeNodesDegree[id] = 0;
		if (activeNodesLabeling[id] > 0)
			activeNodesDegree[id] = outDegree[id];
	}
}
...
__global__ void makeQueueUniform(unsigned int *activeNodes, unsigned int *activeNodesLabeling,
						  unsigned int *prefixLabeling, unsigned int numNodes)
{
	unsigned int id = blockDim.x * blockIdx.x + threadIdx.x;
	if (id < numNodes && activeNodesLabeling[id] > 0)
	{
		uint from = prefixLabeling[id];
		uint to = from + activeNodesLabeling[id];
...
__global__ void makeActiveNodesPointerUniform(unsigned int *activeNodesPointer, unsigned int *activeNodesLabeling,
											  unsigned int *prefixLabeling, unsigned int *prefixSumDegrees, unsigned int *outDegree,
											  unsigned int numNodes, uint maxDegreeThreshold)
{
	unsigned int id = blockDim.x * blockIdx.x + threadIdx.x;
	if (id < numNodes && activeNodesLabeling[id] > 0)
	{
		uint from = prefixLabeling[id];
		uint to = prefixLabeling[id] + activeNodesLabeling[id];
		uint unit = outDegree[id] / maxDegreeThreshold;
		for(uint i = from; i < to; i++)
			activeNodesPointer[i] = prefixSumDegrees[id] + (i - from) * unit;
	}
}
...
// pthread
template <class E, class EL, class DL>
void dynamic_uniform(unsigned int tId,
			 unsigned int numThreads,
			 unsigned int numActiveNodes,
			 unsigned int *activeNodes,
			 unsigned int *outDegree,
			 ...
			 E *edgeList)
{
	...
	for (unsigned int i = left; i < right; i++)
	{
		if (i > 0 && activeNodes[i - 1] == activeNodes[i])
			continue;
		thisNode = activeNodes[i];
		thisDegree = outDegree[thisNode];
		fromHere = activeNodesPointer[i];
...
void SubgraphGenerator<E, EL, DL>::generate(Evolving_Graph<E, EL, DL> &graph, Subgraph<E> &subgraph)
{
	...
	// cout << "last active node: " << subgraph.activeNodes[subgraph.numActiveNodes - 1] << endl;
	// time = t.Finish();
...
template <class E, class EL, class DL>
void SubgraphGenerator<E, EL, DL>::generate_uniform(Evolving_Graph<E, EL, DL> &graph, Subgraph<E> &subgraph, uint maxDegreeThreshold)
{
	prePrefix<<<graph.num_nodes / 512 + 1, 512>>>(d_activeNodesLabeling, d_activeNodesDegree, graph.d_outDegree, graph.d_label1, graph.d_label2, graph.num_nodes, maxDegreeThreshold);
	thrust::device_ptr<unsigned int> ptr_labeling(d_activeNodesLabeling);
	thrust::device_ptr<unsigned int> ptr_labeling_prefixsum(d_prefixLabeling);
	...
	makeQueueUniform<<<graph.num_nodes / 512 + 1, 512>>>(subgraph.d_activeNodes, d_activeNodesLabeling, d_prefixLabeling, graph.num_nodes);
	gpuErrorcheck(cudaMemcpy(subgraph.activeNodes, subgraph.d_activeNodes, subgraph.numActiveNodes * sizeof(unsigned int), cudaMemcpyDeviceToHost));
	...
	makeActiveNodesPointerUniform<<<graph.num_nodes / 512 + 1, 512>>>(subgraph.d_activeNodesPointer, d_activeNodesLabeling, d_prefixLabeling, d_prefixSumDegrees, graph.d_outDegree, graph.num_nodes, maxDegreeThreshold);
	gpuErrorcheck(cudaMemcpy(subgraph.activeNodesPointer, subgraph.d_activeNodesPointer, subgraph.numActiveNodes * sizeof(unsigned int), cudaMemcpyDeviceToHost));
	...
	for (unsigned int t = 0; t < numThreads; t++)
	{
		runThreads[t] = thread(dynamic_uniform<E, EL, DL>,
							   t,
							   numThreads,
							   subgraph.numActiveNodes,
							   subgraph.activeNodes,
