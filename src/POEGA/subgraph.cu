...
template <class E>
Subgraph<E>::Subgraph(uint num_nodes, uint num_edges, ull max_partition_size_)
{	
	/*
	cudaProfilerStart();
	cudaError_t error;
	...
	*/
	max_partition_size = max_partition_size_;
...
template <class E>
Subgraph<E>::~Subgraph(){
	cudaFreeHost(activeNodes);
	cudaFreeHost(activeNodesPointer);
	cudaFreeHost(activeEdgeList);

	cudaFree(d_activeNodes);
	cudaFree(d_activeNodesPointer);
	cudaFree(d_activeEdgeList);
}
