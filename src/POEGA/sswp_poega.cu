...
void two_stage_analysis(Evolving_Graph<OutEdgeWeighted_Evolving, EdgeWeighted_Union, EdgeWeighted> &graph, ArgumentParser& arguments, int seed){
	...
	while (graph.numActiveNodes > 0)
	{
		...
		sswp_kernel_common_warp<<< (graph.numActiveNodes * WARP_SIZE) / 512 + 1, 512>>>(graph.numActiveNodes,
	...
	#pragma omp parallel for
		...
		gpuErrorcheck(cudaPeekAtLastError());
		sub_timer.Start();
		sswp_incremental_cg_concurrent2<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								d_values,
																								graph.d_label2,
		...
		while (graph.numActiveNodes > 0)
		{
			...
			timer_sub2.Start();
			moveUpLabels<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			sswp_kernel_concurrent_optimized_transpose<<<(graph.numActiveNodes * 32) / 512 + 1, 512>>>(graph.numActiveNodes,
																									graph.d_nodePointer_core,
																									graph.d_edgeList_core,
																									graph.d_activeNodes,
																									graph.d_label2,
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			extend_varray_to_edgelist<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionNodeSize[pi],
																									...
																									d_nodePointer_el,
																									graph.d_outDegree);
			sswp_incre_concurrent_locality_bound<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											d_values,
																											d_max,
																											...
																											graph.d_label2,
																											graph.numSnapshots);
			sswp_incre_concurrent2_locality<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											d_values,
																											graph.d_label2,
		...
		gpuErrorcheck(cudaPeekAtLastError());
		sub_timer.Start();
		sswp_incremental_cg_concurrent2_newstorage<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_value,
																								d_buffer,
		...
		graph.calCurNumActiveNodes();
		float tracing_time = sub_timer.Finish();
		cout << ">Phase 1: Tracing: " << tracing_time << " (ms).\n";
		...
		while (graph.numActiveNodes > 0)
		{
			...
			timer_sub2.Start();
			moveUpLabels<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			sswp_kernel_concurrent_newstorage<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_label1,
																								graph.d_label2,
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			extend_varray_to_edgelist<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionNodeSize[pi],
																									...
																									d_nodePointer_el,
																									graph.d_outDegree);
			sswp_incre_concurrent_locality_bound_newstorage<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											graph.d_value,
																											d_buffer,
																											...
																											d_total,
																											graph.numSnapshots);
			sswp_incre_concurrent2_locality_newstorage<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											graph.d_value,
																											d_buffer,
