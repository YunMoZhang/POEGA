...
void two_stage_analysis(Evolving_Graph<OutEdgeWeighted_Evolving, EdgeWeighted_Union, EdgeWeighted> &graph, ArgumentParser& arguments, int seed){
	...
	while (graph.numActiveNodes > 0)
	{
		...
		sssp_kernel_common_warp<<< (graph.numActiveNodes * WARP_SIZE) / 512 + 1, 512>>>(graph.numActiveNodes,
	...
	// gpuErrorcheck(cudaMemcpy(graph.depends_core, graph.d_depends, graph.num_nodes * sizeof(DependencyData), cudaMemcpyDeviceToHost));
	...
	// Check available GPU memory before allocating the snapshot value array.
	...
	cout << "GPU memory Free: " << free_mem << " bytes, value arrays require: " << value_bytes << " bytes\n";
	if (value_bytes < free_mem * 0.8) 
	{
		uint *value_data = new uint[(size_t)graph.numSnapshots * graph.num_nodes];
		
	#pragma omp parallel for
		for (uint i = 0; i < graph.num_nodes; i++)
		{
			size_t base = (size_t)i * graph.numSnapshots;
			for (uint j = 0; j < graph.numSnapshots; j++)
			{
				value_data[base + j] = graph.value[i];
			}
		}

		std::cout << "Transferring values to device..." << std::endl;

		uint *d_values;
		gpuErrorcheck(cudaMalloc(&d_values, (size_t)graph.num_nodes * graph.numSnapshots * sizeof(uint)));
		// cout << (size_t)graph.num_nodes * graph.numSnapshots * sizeof(uint) << endl;
		gpuErrorcheck(cudaMemcpy(d_values, value_data, (size_t)graph.num_nodes * graph.numSnapshots * sizeof(uint), cudaMemcpyHostToDevice));
		...
		gpuErrorcheck(cudaPeekAtLastError());
		sub_timer.Start();
		sssp_incremental_cg_concurrent_bound<<<graph.num_nodes / 512 + 1, 512>>>(graph.num_nodes,
																		graph.d_nodePointer_core,
																		graph.d_edgeList_core,
																		graph.d_value,
																		d_values,
		...
		while (graph.numActiveNodes > 0)
		{
			...
			timer_sub2.Start();
			moveUpLabels<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			sssp_kernel_concurrent_optimized_transpose<<<(graph.numActiveNodes * WARP_SIZE) / 512 + 1, 512>>>(graph.numActiveNodes,
																									graph.d_nodePointer_core,
																									graph.d_edgeList_core,
																									graph.d_activeNodes,
																									graph.d_label2,
		...
		gpuErrorcheck(cudaMemcpy(value_data, d_values, (size_t)graph.num_nodes * graph.numSnapshots * sizeof(uint), cudaMemcpyDeviceToHost));
		...
		uint *d_max, *d_min;
		uint * max = new uint[graph.num_nodes];
		uint * min = new uint[graph.num_nodes];
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			extend_varray_to_edgelist<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionNodeSize[pi],
																									...
																									d_nodePointer_el,
																									graph.d_outDegree);
			sssp_incre_concurrent_locality_bound<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											d_values,
																											graph.d_label2,
																											...
																											d_min,
																											graph.numSnapshots);
			sssp_incre_concurrent2_locality<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																											d_nodePointer_el,
																											d_edgeList,
																											d_values,
																											graph.d_label2,
		...
		cout << "P2 Incre time: " << total_time << endl;
		...
		while (graph.numActiveNodes > 0){
			...
			moveUpLabels<<< graph.num_nodes/512 + 1, 512 >>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			sssp_kernel_concurrent_optimized_transpose<<<(graph.numActiveNodes * 32) / 512 + 1, 512>>>(graph.numActiveNodes,
																								graph.d_nodePointer,
																								graph.d_edgeList,
																								graph.d_activeNodes,
																								graph.d_label2,
	...
	{ // Using AMVA for compressing multi-version state data
		cout << "Using AMVA for compressing multi-version state data..." << endl;
		cudaFree(d_prefixLabeling);
		cudaFree(graph.d_activeNodes);
		gpuErrorcheck(cudaPeekAtLastError());

		uint *d_buffer;
		uint h_total = (graph.num_nodes * 0.55);
		...
		gpuErrorcheck(cudaPeekAtLastError());
		sub_timer.Start();
		sssp_incremental_cg_concurrent2_newstorage_waitfree<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_value,
																								d_buffer,
		...
		while (graph.numActiveNodes > 0)
		{
			...
			timer_sub2.Start();
			moveUpLabels<<<graph.num_nodes / 512 + 1, 512>>>(graph.d_label1, graph.d_label2, graph.num_nodes);
			sssp_kernel_concurrent_newstorage_waitfree<<<(graph.num_nodes * graph.numSnapshots) / 512 + 1, 512>>>(graph.num_nodes,
																								graph.d_nodePointer_core,
																								graph.d_edgeList_core,
																								graph.d_label1,
																								graph.d_label2,
		...
		gpuErrorcheck(cudaMalloc(&d_max, graph.num_nodes * sizeof(uint)));
		gpuErrorcheck(cudaMalloc(&d_min, graph.num_nodes * sizeof(uint)));
		set_max_min_waitfree<<<graph.num_nodes / 512 + 1, 512>>>(d_max, d_min, graph.d_value, d_buffer, d_total, graph.num_nodes, graph.numSnapshots);
		gpuErrorcheck(cudaPeekAtLastError());
		...
		for (int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			extend_varray_to_edgelist<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionNodeSize[pi],
																									...
																									d_nodePointer_el,
																									graph.d_outDegree);
			sssp_incre_concurrent_locality_bound_newstorage_waitfree<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																										d_nodePointer_el,
																										d_edgeList,
																										graph.d_value,
																										d_buffer,
																										...
																										d_total,
																										graph.numSnapshots);
			sssp_incre_concurrent2_locality_newstorage_waitfree<<<partitioner.partitionEdgeSize[pi] / 512 + 1, 512, 0, stream>>>(partitioner.partitionEdgeSize[pi],
																										d_nodePointer_el,
																										d_edgeList,
																										graph.d_value,
																										d_buffer,
...
void preAnalysisFull(Evolving_Graph<OutEdgeWeighted_Evolving, EdgeWeighted_Union, EdgeWeighted> &graph){
	...
	for(uint top_i = 0; top_i < graph.top_k_degree; top_i++){
		...
		graph.depends[graph.top_k_degree_nodes[top_i]].value = 0;
		...
		while (subgraph.numActiveNodes > 0)
		{
			...
			for (int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				sssp_kernel<<<partitioner.partitionNodeSize[pi] / 512 + 1, 512>>>(partitioner.partitionNodeSize[pi],
																				partitioner.fromNode[pi],
																				partitioner.fromEdge[pi],
