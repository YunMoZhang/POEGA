...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			sswp_kernel_ks<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	for(uint i = 1; i <= graph.numSnapshots; i++)
	{
		...
		while (subgraph.numActiveNodes > 0)
		{
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				sswp_jumpstart<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				sswp_propogate<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
		...
		for(int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			sswp_pull_once<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
															partitioner.fromNode[pi],
															partitioner.fromEdge[pi],
															subgraph.d_activeNodes,
															subgraph.d_activeNodesPointer,
