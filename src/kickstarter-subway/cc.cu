...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			cc_kernel_ks<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	cout << "CC Processing finished in " << runtime << " (ms).\n";
	...
	for(uint i = 1; i <= graph.numSnapshots; i++)
	{
		...
		timer_sub.Start();
		cc_identifyDel<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_edges_del, 
		...
		while (subgraph.numActiveNodes > 0)
		{
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				cc_jumpstart<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				cc_propogate<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
		...
		for(int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			cc_pull_once<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
															partitioner.fromNode[pi],
															partitioner.fromEdge[pi],
															subgraph.d_activeNodes,
															subgraph.d_activeNodesPointer,
		...
		timer_sub.Start();
		cc_incre<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_edges_add, 
		...
		gpuErrorcheck(cudaMemcpy(graph.depends, graph.d_depends, graph.num_nodes * sizeof(DependencyData), cudaMemcpyDeviceToHost));
		utilities::PrintResults(graph.depends, min(30, graph.num_nodes));
		// cout << "Total transfer: " << transfer_total << "\t ratios: " << (float)transfer_total / graph.num_edges_total << endl; 
	}
}