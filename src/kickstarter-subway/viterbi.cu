...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			viterbi_kernel_ks<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	cout << "Viterbi Processing finished in " << runtime << " (ms).\n";
	...
	for(uint i = 1; i <= graph.numSnapshots; i++)
	{
		...
		timer_sub.Start();
		viterbi_identifyDel<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_edges_del, 
		...
		while (subgraph.numActiveNodes > 0)
		{
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				viterbi_jumpstart<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				viterbi_propogate<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
		...
		for(int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			viterbi_pull_once<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
															partitioner.fromNode[pi],
															partitioner.fromEdge[pi],
															subgraph.d_activeNodes,
															subgraph.d_activeNodesPointer,
		...
		timer_sub.Start();
		viterbi_incre<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_edges_add, 
