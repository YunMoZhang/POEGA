...
int main(int argc, char** argv)
{
	...
	while (subgraph.numActiveNodes>0)
	{
		...
		{	
			...
			timer_sub2.Start();
			bfs_kernel_ks<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
	...
	cout << "BFS Processing finished in " << runtime << " (ms).\n";
	...
	for(uint i = 1; i <= graph.numSnapshots; i++)
	{
		...
		timer_sub.Start();
		bfs_identifyDel<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_edges_del, 
		...
		while (subgraph.numActiveNodes > 0)
		{
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				bfs_jumpstart<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
			...
			for(int pi = 0; pi < partitioner.numPartitions; pi++)
			{
				...
				timer_sub2.Start();
				bfs_propogate<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
																partitioner.fromNode[pi],
																partitioner.fromEdge[pi],
																subgraph.d_activeNodes,
																subgraph.d_activeNodesPointer,
		...
		for(int pi = 0; pi < partitioner.numPartitions; pi++)
		{
			...
			timer_sub2.Start();
			bfs_pull_once<<< partitioner.partitionNodeSize[pi]/512 + 1 , 512 >>>(partitioner.partitionNodeSize[pi],
															partitioner.fromNode[pi],
															partitioner.fromEdge[pi],
															subgraph.d_activeNodes,
															subgraph.d_activeNodesPointer,
		...
		timer_sub.Start();
		bfs_incre<<< graph.num_nodes/512 + 1, 512 >>>(graph.num_edges_add, 
