...
// #define WEIGHT(x, y) ((x + y) % 50 + 1) // for most dataset
#define WEIGHT(x, y) ((x + y) % 20 + 1)
...
void Evolving_Graph<E, EL, DL>::CreateCSR(uint num_edges, uint *nodePointer, E *edgeList, int sid_a[], int sid_d[])
{
	...
	memset(outDegree, 0, num_nodes * sizeof(uint));
	for(uint i=0; i<num_edges; i++){
		pos_arr[i] = outDegree[edgesInput[i].source]++;
	}
...
#pragma omp parallel for
	for(uint i=0; i<num_edges; i++)
	{
		...
		location = nodePointer[edgesInput[i].source] + pos_arr[i];
...
void Evolving_Graph<E, EL, DL>::ReadGraph(bool fileContainsWeight, int seed)
{
	...
	if(graphFormat == "bin")
	{
		...
		num_edges_input *= 2;
		cout << "Finished file reading and all edges are stored (" << num_nodes << " Nodes, " << num_edges_input << " Edges) ...\n";
	...
	gpuErrorcheck(cudaMalloc(&d_all_affected_vertices, num_nodes * sizeof(bool)));
	...
	if(num_edges_add % 2 != 0) num_edges_add -= 1;  // ensuring addition delta is symmetric
	num_edges_total = num_edges_base + numSnapshots * num_edges_add;
	num_edges_del = num_edges_input * delta_rate_del;
	if(num_edges_del % 2 != 0) num_edges_del -= 1;  // ensuring deletion delta is symmetric
	...
	gpuErrorcheck(cudaMallocManaged(&d_edgeList, num_edges_total * sizeof(E)));
...
void Evolving_Graph<E, EL, DL>::copy_to_device_values()
{
	...
	gpuErrorcheck(cudaMemcpy(d_depends, depends, num_nodes * sizeof(DependencyData), cudaMemcpyHostToDevice));
}

template <class E, class EL, class DL>
void Evolving_Graph<E, EL, DL>::copy_to_device_delta(uint i)
{
	gpuErrorcheck(cudaMemcpy(d_add_edgeList, add_edgeList[i], num_edges_add * sizeof(DL), cudaMemcpyHostToDevice));
	gpuErrorcheck(cudaMemcpy(d_del_edgeList, del_edgeList[i], num_edges_del * sizeof(DL), cudaMemcpyHostToDevice));
}
